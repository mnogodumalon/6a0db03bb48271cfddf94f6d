...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/AllgemeineEingabe';
...
interface AllgemeineEingabeDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: AllgemeineEingabe['fields']) => Promise<void>;
  defaultValues?: AllgemeineEingabe['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function AllgemeineEingabeDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: AllgemeineEingabeDialogProps) {
  const [fields, setFields] = useState<Partial<AllgemeineEingabe['fields']>>({});
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults((defaultValues ?? {}) as Record<string, unknown>, formEnhancements.defaults) as Partial<AllgemeineEingabe['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'allgemeine_eingabe');
      await onSubmit(clean as AllgemeineEingabe['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<AllgemeineEingabe['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Allgemeine Eingabe bearbeiten' : 'Allgemeine Eingabe hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'titel': (
      <div key="titel" className="space-y-1.5">
        ...
        <Input
          id="titel"
          placeholder="z. B. Projektupdate März"
          value={fields.titel ?? ''}
    ...
    'beschreibung': (
      <div key="beschreibung" className="space-y-1.5">
        ...
        <Textarea
          id="beschreibung"
          placeholder="Hintergrund, Details, Notizen..."
  ...
  const FIELD_LABELS: Record<string, string> = {"titel": "Titel", "beschreibung": "Beschreibung", "datum": "Datum"};
