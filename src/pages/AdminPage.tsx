...
import { AllgemeineEingabeViewDialog } from '@/components/dialogs/AllgemeineEingabeViewDialog';
...
const ALLGEMEINEEINGABE_FIELDS = [
...
const ENTITY_TABS = [
  { key: 'allgemeine_eingabe', label: 'Allgemeine Eingabe', pascal: 'AllgemeineEingabe' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('allgemeine_eingabe');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'allgemeine_eingabe': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'allgemeine_eingabe': return (data as any).allgemeineEingabe as AllgemeineEingabe[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'allgemeine_eingabe': return ALLGEMEINEEINGABE_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'allgemeine_eingabe': return {
        create: (fields: any) => LivingAppsService.createAllgemeineEingabeEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateAllgemeineEingabeEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteAllgemeineEingabeEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'allgemeine_eingabe' || dialogState?.entity === 'allgemeine_eingabe') && (
        <AllgemeineEingabeDialog
          open={createEntity === 'allgemeine_eingabe' || dialogState?.entity === 'allgemeine_eingabe'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'allgemeine_eingabe' ? handleUpdate : (fields: any) => handleCreate('allgemeine_eingabe', fields)}
          defaultValues={dialogState?.entity === 'allgemeine_eingabe' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['AllgemeineEingabe']}
          enablePhotoLocation={AI_PHOTO_LOCATION['AllgemeineEingabe']}
        />
      )}
      {viewState?.entity === 'allgemeine_eingabe' && (
        <AllgemeineEingabeViewDialog
          open={viewState?.entity === 'allgemeine_eingabe'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'allgemeine_eingabe', record: r }); }}
        />
      )}
