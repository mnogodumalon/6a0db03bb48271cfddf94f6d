...
export default function AllgemeineEingabePage() {
  const [records, setRecords] = useState<AllgemeineEingabe[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<AllgemeineEingabe | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<AllgemeineEingabe | null>(null);
  const [viewingRecord, setViewingRecord] = useState<AllgemeineEingabe | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getAllgemeineEingabe());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: AllgemeineEingabe['fields']) {
    await LivingAppsService.createAllgemeineEingabeEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: AllgemeineEingabe['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateAllgemeineEingabeEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  return (
    <PageShell
      title="Allgemeine Eingabe"
      subtitle={`${records.length} Allgemeine Eingabe im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Allgemeine Eingabe suchen..."
          value={search}
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={4} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Allgemeine Eingabe. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
      ...
      <ConfirmDialog
        ...
        title="Allgemeine Eingabe löschen"
