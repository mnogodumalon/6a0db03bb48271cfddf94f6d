...
import type { AllgemeineEingabe, CreateAllgemeineEingabe } from '@/types/app';
...
export class LivingAppsService {
  // --- ALLGEMEINE_EINGABE ---
  static async getAllgemeineEingabe(): Promise<AllgemeineEingabe[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.ALLGEMEINE_EINGABE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as AllgemeineEingabe[];
    return enrichLookupFields(records, 'allgemeine_eingabe');
  }
  static async getAllgemeineEingabeEntry(id: string): Promise<AllgemeineEingabe | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.ALLGEMEINE_EINGABE}/records/${id}`);
    const record = { record_id: data.id, ...data } as AllgemeineEingabe;
    return enrichLookupFields([record], 'allgemeine_eingabe')[0];
  }
  static async createAllgemeineEingabeEntry(fields: CreateAllgemeineEingabe) {
    return callApi('POST', `/apps/${APP_IDS.ALLGEMEINE_EINGABE}/records`, { fields: cleanFieldsForApi(fields as any, 'allgemeine_eingabe') });
  }
  static async updateAllgemeineEingabeEntry(id: string, fields: Partial<CreateAllgemeineEingabe>) {
    return callApi('PATCH', `/apps/${APP_IDS.ALLGEMEINE_EINGABE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'allgemeine_eingabe') });
  }
  static async deleteAllgemeineEingabeEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.ALLGEMEINE_EINGABE}/records/${id}`);
  }
